...
export const getNotes = async (): Promise<KnowledgeNote[]> => {
  const { data } = await api.get('/knowledge/notes');
  return data;
};

export const createNote = async (noteData: NoteCreate): Promise<KnowledgeNote> => {
  const { data } = await api.post('/knowledge/notes', noteData);
  return data;
};

export const deleteNote = async (noteId: string): Promise<{ status: string }> => {
  const { data } = await api.delete(`/knowledge/notes/${noteId}`);
  return data;
};

export const generateInfographic = async (infographicData: { document_ids: string[]; session_id?: string; style?: string }) => {
  const { data } = await api.post('/knowledge/infographic', infographicData);
  return data;
};
